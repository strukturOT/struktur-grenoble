  ...
  return (
    <div className="min-h-screen overflow-hidden pt-24 md:pt-32">
      ...
      <section className="mx-auto max-w-7xl px-6 pt-16 md:px-12 md:pt-24">
        ...
        {categoriesLoading ? <div className="grid grid-cols-2 gap-3 md:grid-cols-4">{[0, 1, 2, 3].map((item) => <div key={item} className="aspect-[1.15] animate-pulse bg-theme-surface" />)}</div> : categories.length > 0 ? <div className="grid grid-cols-2 gap-3 md:grid-cols-4">{categories.map((category) => <Link key={category.id} to={`/boutique?categorie=${category.slug}#selection`} className={`group relative aspect-[1.15] overflow-hidden bg-theme-surface ${activeCategory?.id === category.id ? 'ring-2 ring-struktur-orange ring-offset-2 ring-offset-theme-canvas' : ''}`}><div className="absolute inset-0 bg-black/20 transition-colors duration-500 group-hover:bg-black/5">{category.imageUrl && <img src={category.imageUrl} alt="" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />}</div><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/10 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-4 text-white md:p-5"><p className="text-[10px] uppercase tracking-[0.18em] text-white/70">{category.productCount} pièce{category.productCount !== 1 ? 's' : ''}</p><p className="mt-1 font-display text-xl md:text-2xl">{category.name}</p></div></Link>)}</div> : null}
      </section>
