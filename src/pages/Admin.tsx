...
import { createCategory, createProduct, deleteCategory, formatMoney, getAdminCategories, getAdminCustomers, getAdminMessages, getAdminOrders, getAdminProducts, productImage, updateCategory, type ContactMessage, type ProductStatus, type ProductVariant, type StoreCategory, type StoreCustomer, type StoreOrder, type StoreProduct, updateCustomerRole, updateMessageStatus, updateProduct, updateProductStatus } from '../lib/commerce';
import { AdminOperations } from '../components/AdminOperations';
import { formatFileSize } from '../lib/imageOptimization';

type VariantDraft = { id: string; variantId?: string; name: string; sku: string; price: string; stock: string; attributes: ProductVariant['attributes'] };
export type AdminPage = 'overview' | 'catalogue' | 'categories' | 'orders' | 'messages' | 'clients' | 'access';

const createVariant = (): VariantDraft => ({ id: crypto.randomUUID(), name: '', sku: '', price: '', stock: '0', attributes: {} });
...
const ProductPanel = ({ product, categories, onClose, onSaved }: { product: StoreProduct | null; categories: StoreCategory[]; onClose: () => void; onSaved: () => Promise<void> }) => {
  ...
  const updateVariant = (id: string, field: 'name' | 'sku' | 'price' | 'stock' | 'colorway', value: string) => setVariants((current) => current.map((variant) => {
    if (variant.id !== id) return variant;
    if (field === 'colorway') return { ...variant, attributes: { ...variant.attributes, colorway: value } };
    if (field === 'name') return { ...variant, name: value, attributes: { ...variant.attributes, ...(variant.attributes.size ? { size: value } : {}) } };
  ...
  const variantGroups = useMemo(() => {
    ...
    variants.forEach((variant) => {
      const colorway = variant.attributes.colorway?.trim() || 'Autres variantes';
      groups.set(colorway, [...(groups.get(colorway) ?? []), variant]);
    });
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const input = {
        ...
        variants: variants.map((variant) => ({ id: variant.variantId, name: variant.name.trim(), sku: variant.sku.trim(), priceCents: variant.price ? Math.round(Number(variant.price) * 100) : null, stockQuantity: Number(variant.stock), attributes: { ...variant.attributes, ...(variant.attributes.size ? { size: variant.name.trim() } : {}) } })),
