...
import { AlertTriangle, ArrowUpRight, Box, CheckCircle2, ImagePlus, Layers3, LayoutDashboard, Mail, Package, Plus, RefreshCw, ShieldCheck, ShoppingBag, UsersRound, X } from 'lucide-react';
import { NavLink } from 'react-router-dom';
import { useAuth } from '../auth/AuthProvider';
import { createProduct, formatMoney, getAdminCustomers, getAdminMessages, getAdminOrders, getAdminProducts, productImage, type ContactMessage, type ProductStatus, type StoreCustomer, type StoreOrder, type StoreProduct, updateCustomerRole, updateMessageStatus, updateProduct, updateProductStatus } from '../lib/commerce';
import { AdminOperations } from '../components/AdminOperations';

type VariantDraft = { id: string; variantId?: string; name: string; sku: string; price: string; stock: string };
export type AdminPage = 'overview' | 'catalogue' | 'orders' | 'messages' | 'clients' | 'access';

const createVariant = (): VariantDraft => ({ id: crypto.randomUUID(), name: '', sku: '', price: '', stock: '0' });
const createInitialForm = () => ({ name: '', slug: '', brand: '', description: '', price: '', currency: 'EUR', status: 'draft' as ProductStatus, featured: false });
...
const Admin = ({ page = 'overview' }: { page?: AdminPage }) => {
  ...
  const loadDashboard = useCallback(async () => {
    ...
    const [productResult, orderResult, customerResult, messageResult] = await Promise.all([
    ...
    <div className="min-h-screen bg-[#0b0b0c] text-[#f2f0ec]">
      ...
      <main className="mx-auto max-w-[1560px] px-5 pb-16 pt-36 md:ml-[248px] md:px-10 md:pt-12 xl:px-14">
        ...
        {page === 'catalogue' && <section className="py-4">
          <SectionHeading eyebrow="Inventaire" title="Catalogue" detail={`${products.length} produit${products.length > 1 ? 's' : ''}`} action={<button type="button" onClick={() => setPanelProduct(null)} className="hidden items-center gap-2 border border-white/20 px-4 py-2.5 text-[10px] font-medium uppercase tracking-[0.16em] transition-colors hover:border-struktur-orange hover:text-struktur-orange sm:inline-flex"><Plus size={14} />Ajouter</button>} />
      ...
      <AnimatePresence>{panelProduct !== undefined && <ProductPanel product={panelProduct} onClose={() => setPanelProduct(undefined)} onSaved={async () => { setPanelProduct(undefined); await loadDashboard(); }} />}</AnimatePresence>
    </div>
  );
};
...
const AdminNavigation = ({ adminName, onRefresh, isLoading }: { adminName: string | null | undefined; onRefresh: () => void; isLoading: boolean }) => <>
  <aside className="fixed inset-x-0 top-0 z-40 flex h-[68px] items-center justify-between border-b border-white/10 bg-[#0b0b0c]/95 px-5 backdrop-blur-md md:inset-y-0 md:left-0 md:right-auto md:h-screen md:w-[248px] md:flex-col md:items-stretch md:border-b-0 md:border-r md:px-6 md:py-8">
    ...
    <nav className="absolute left-0 top-full flex w-screen overflow-x-auto border-b border-white/10 bg-[#0b0b0c]/95 px-2 py-1 backdrop-blur-md md:static md:block md:w-auto md:space-y-1 md:border-0 md:bg-transparent md:px-0 md:py-0"><AdminNavItem to="/admin" end icon={<LayoutDashboard size={16} />} label="Vue d’ensemble" /><AdminNavItem to="/admin/catalogue" icon={<Layers3 size={16} />} label="Catalogue" /><AdminNavItem to="/admin/commandes" icon={<ShoppingBag size={16} />} label="Commandes" /><AdminNavItem to="/admin/messages" icon={<Mail size={16} />} label="Messages" /><AdminNavItem to="/admin/clients" icon={<UsersRound size={16} />} label="Clients" /><AdminNavItem to="/admin/acces" icon={<ShieldCheck size={16} />} label="Accès équipe" /></nav>
...
const ProductPanel = ({ product, onClose, onSaved }: { product: StoreProduct | null; onClose: () => void; onSaved: () => Promise<void> }) => {
